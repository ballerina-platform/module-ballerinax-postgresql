...
function TestLinkFailure() {
    ...
    test:assertTrue(strings:includes(sqlerror.message(), "Error in SQL connector configuration: Failed to initialize pool: The connection " +
            "attempt failed. Caused by :The connection attempt failed. Caused by :host"), sqlerror.message());
}
...
function TestInvalidDB() {
    ...
    test:assertTrue(dbClient is sql:ApplicationError);
    sql:ApplicationError sqlerror = <sql:ApplicationError> dbClient;
    test:assertTrue(strings:includes(sqlerror.message(), "Error in SQL connector configuration: Failed to initialize pool: FATAL: " +
            "database \"errorD\" does not exist Caused by :FATAL: database \"errorD\" does not exist"), sqlerror.message());
}
...
function TestInvalidTableName() returns error? {
    ...
    test:assertTrue(strings:includes(sqlerror.message(), "Error while executing SQL query: SELECT string_type from Data WHERE row_id = 1. ERROR: " +
                "relation \"data\" does not exist"), sqlerror.message());
}
